...
class _StickerEditorState extends State<StickerEditor> {
  ...
  Future<void> _share() => _run((png) async {
        ...
        final file = File('${dir.path}/AcademiaApp-sticker.png');
  ...
  Future<void> _save() => _run((png) async {
        final ok = await saveImageToGallery(png, 'AcademiaApp-${DateTime.now().millisecondsSinceEpoch}.png');
...
class _Sticker extends StatelessWidget {
  ...
  Widget _mark({double size = 12}) => Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('AcademiaApp', style: _s(size, FontWeight.w900, alpha: 0.9, spacing: 0.2)),
      ]);
